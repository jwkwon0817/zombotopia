...
from zombotopia.core.schemas.ranking import RankingModel, UserRankingModel
...
@router.get('/{user_id}')
async def get_ranking(user_id: str, db: Session = Depends(get_db)) -> UserRankingModel | ErrorSchema:
    try:
        ranking_service = RankingService(db)
        ranking = ranking_service.get_ranking_by_id(user_id)
        return UserRankingModel(
            user_id=ranking[0].user_id,
